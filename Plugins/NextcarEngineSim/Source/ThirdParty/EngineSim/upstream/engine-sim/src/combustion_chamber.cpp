...
void CombustionChamber::initialize(const Parameters &params) {
    ...
    setDeterministicSeed(params.randomSeed);
    ...
    m_system.setGeometry(m_cylinderWidthApproximation, height, 1.0, 0.0);
    ...
    m_intakeRunnerAndManifold.initialize(units::pressure(1.0, units::atm), totalIntakeRunnerVolume, units::celcius(25.0));
    m_intakeRunnerAndManifold.setGeometry(overallIntakeRunnerLength, intakeRunnerWidth, 1.0, 0.0);
    ...
    const double exhaustTubeLength = exhaust->getPrimaryTubeLength() + m_head->getHeaderPrimaryLength(m_piston->getCylinderIndex());
    ...
    m_exhaustRunnerAndPrimary.initialize(units::pressure(1.0, units::atm), totalExhaustRunnerVolume, units::celcius(25.0));
    m_exhaustRunnerAndPrimary.setGeometry(overallExhaustRunnerLength, exhaustRunnerWidth, 1.0, 0.0);
}
...
double CombustionChamber::getVolume() const {
    ...
    const double s = m_piston->relativeX() * bank->getDx() + m_piston->relativeY() * bank->getDy();
    const double sweep = area * (bank->getDeckHeight() - s - m_piston->getCompressionHeight());
    return sweep + combustionPortVolume - m_piston->getDisplacement();
}

double CombustionChamber::pistonSpeed() const {
    const CylinderBank *bank = m_head->getCylinderBank();
    return m_piston->m_body.v_x * bank->getDx() + m_piston->m_body.v_y * bank->getDy();
}

double CombustionChamber::calculateMeanPistonSpeed() const {
    double avg = 0;
    for (int i = 0; i < StateSamples; ++i) avg += m_pistonSpeed[i];
    return avg / StateSamples;
}

double CombustionChamber::calculateFiringPressure() const {
    double firingPressure = 0;
    for (int i = 0; i < StateSamples; ++i) firingPressure = std::fmax(firingPressure, m_pressure[i]);
...
double CombustionChamber::nextUnitRandom() {
    std::uint32_t x = m_rngState;
    x ^= x << 13;
    x ^= x >> 17;
    x ^= x << 5;
    m_rngState = x == 0 ? 1u : x;
    return static_cast<double>(m_rngState) / static_cast<double>(UINT32_MAX);
}
...
void CombustionChamber::ignite() {
    if (!m_lit) {
        ...
        if (equivalenceRatio < 0.5 || equivalenceRatio > 1.9) return;
        ...
        const double randomness = m_fuel->getBurningEfficiencyRandomness();
        const double lowEfficiencyAttenuation = m_fuel->getLowEfficiencyAttenuation();
        const double maxBurningEfficiency = m_fuel->getMaxBurningEfficiency();
        const double maxTurbulenceEffect = m_fuel->getMaxTurbulenceEffect();
        const double maxDilutionEffect = m_fuel->getMaxDilutionEffect();
        const double turbulence = m_meanPistonSpeedToTurbulence->sampleTriangle(calculateMeanPistonSpeed());
        const double mixingFactor = 1.0 - (clamp(turbulence / maxTurbulenceEffect) * clamp(1 - dilution / maxDilutionEffect));
        const double rand_s = lowEfficiencyAttenuation * ((1 - randomness) + randomness * nextUnitRandom());
        const double efficiencyAttenuation = mixingFactor * rand_s + (1 - mixingFactor);
        m_flameEvent.efficiency = efficiencyAttenuation * maxBurningEfficiency;
        m_flameEvent.flameSpeed = m_fuel->flameSpeed(turbulence, afr, m_system.temperature(), m_system.pressure(), calculateFiringPressure(), units::pressure(160, units::psi));
    }
}
...
void CombustionChamber::flow(double dt) {
    if (m_system.temperature() > m_peakTemperature) m_peakTemperature = m_system.temperature();
    const double volume = getVolume();
    const double cylinderHeight = volume / m_cylinderCrossSectionSurfaceArea;
    const double cylinderSurfaceArea = cylinderHeight * constants::pi * m_head->getCylinderBank()->getBore() + m_cylinderCrossSectionSurfaceArea * 2;
    ...
    if (std::abs(intakeFlow) > 1E-9 && m_lit) m_lit = false;
    ...
    if (m_lit) {
        ...
        m_flameEvent.travel_x = std::fmin(lastTravel_x + dt * flameSpeed, totalTravel_x);
        m_flameEvent.travel_y = std::fmin(lastTravel_y + dt * flameSpeed, totalTravel_y);
        if (lastTravel_x < m_flameEvent.travel_x || lastTravel_y < m_flameEvent.travel_y) {
            const double burnedVolume = m_flameEvent.travel_x * m_flameEvent.travel_x * constants::pi * m_flameEvent.travel_y;
            const double prevBurnedVolume = lastTravel_x * lastTravel_x * constants::pi * lastTravel_y;
            ...
            m_system.changeEnergy(massFuelBurned * m_fuel->getEnergyDensity());
...
double CombustionChamber::lastEventAfr() const {
    ...
    return totalFuel == 0 ? 0 : (oxygenMolarMass * totalOxygen + totalInert * nitrogenMolarMass) / (totalFuel * octaneMolarMass);
}
...
void CombustionChamber::updateCycleStates() {
    ...
    if (std::isnan(crankAngle) || std::isinf(crankAngle)) crankAngle = 0.0;
    const int i = static_cast<int>(std::round((crankAngle / (4 * constants::pi)) * (StateSamples - 1.0)));
...
void CombustionChamber::apply(atg_scs::SystemState *system) {
    ...
    const double F_fric = v_s > 0 ? -F : F;
    system->applyForce(0.0, 0.0, (force + F_fric) * bank->getDx(), (force + F_fric) * bank->getDy(), m_piston->m_body.index);
}

double CombustionChamber::getFrictionForce() const {
    CylinderBank *bank = m_head->getCylinderBank();
    const double v_s = m_piston->m_body.v_x * bank->getDx() + m_piston->m_body.v_y * bank->getDy();
